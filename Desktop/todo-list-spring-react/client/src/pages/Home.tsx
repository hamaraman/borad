  ...
  if (isListView) {
    return (
      <article
        ref={setNodeRef}
        style={style}
        className={`todo-row ${todo.completed ? "completed" : ""} ${isDragging ? "dragging" : ""}`}
      >
        ...
        <button
          ...
          aria-label={todo.completed ? "미완료로 변경" : "완료 처리"}
        >
          {todo.completed && <Check size={15} strokeWidth={3} />}
        </button>
        <div className="todo-copy" onClick={() => onEdit(todo)}>
...
export default function Home() {
  ...
  const refresh = async () => {
    ...
    setError("");
    try {
      const result = await todosApi.list(page, pageSize, filter, query);
  ...
  const submit = async () => {
    ...
    try {
      if (editing) await todosApi.update(editing.id, form);
      else await todosApi.create(form);
      setShowForm(false);
      toast.success(editing ? "기록을 수정했습니다." : "DB에 기록을 저장했습니다.");
      await refresh();
    } catch (err) {
  ...
  const chooseFilter = (next: "all" | "open" | "done") => {
    setFilter(next);
    setPage(1);
  };
    ...
    <div className="ledger-shell">
      ...
      <main className="ledger-main">
        ...
        <header className="topbar">
          <div>
            <p className="eyebrow">FRIDAY · AUGUST 28, 2026</p>
            <h1>
              오늘할일<span>.</span>
            </h1>
          </div>
        ...
        <section className="intro-row">
          ...
          <div className="stats-strip">
            ...
            <div>
              <span>DUE SOON</span>
              <strong className="coral-text">{String(soonCount).padStart(2, "0")}</strong>
            </div>
            <div>
              <span>COMPLETED</span>
        ...
        <section className="toolbar">
          ...
          <button className="filter-button">
            <SlidersHorizontal size={15} /> 정렬: 마감일순
          </button>
        </section>
        ...
        {viewMode === "list" && (
          <div className="list-heading">
            <span>YOUR RECORDS / {String(totalElements).padStart(2, "0")}</span>
            <span>STATUS · DUE DATE</span>
          </div>
        )}

        {/* Todo List/Grid */}
        <section className={viewMode === "list" ? "todo-list" : "todo-grid"}>
          {loading ? (
            <div className="empty-state">
              <div className="api-loader" />
              <h2>기록을 불러오는 중입니다.</h2>
              <p>Spring Boot API와 동기화하고 있습니다.</p>
            </div>
          ) : error ? (
            <div className="empty-state">
              <h2>API에 연결하지 못했습니다.</h2>
              <p>{error}</p>
              <Button onClick={() => void refresh()}>다시 시도</Button>
            </div>
          ) : displayed.length === 0 ? (
            <div className="empty-state">
              <h2>아직 남겨진 기록이 없습니다.</h2>
              <p>새 기록을 남기고 오늘의 흐름을 시작하세요.</p>
        ...
        <footer className="list-footer">
          <span>
            SHOWING {shownCount} OF {totalElements}
          </span>
      ...
      {showForm && (
        ...
        >
          <div className="record-modal">
            ...
            <h2>{editing ? "기록을 다시 다듬습니다." : "다음 기록을 남깁니다."}</h2>
            <label>
              제목
              <Input
                autoFocus
            ...
            <label>
              ...
              <Textarea
                ...
                placeholder="이 일을 시작하기 전에 기억할 맥락을 적어주세요."
              />
            </label>
            ...
            <Button className="save-button" onClick={() => void submit()}>
              <Sparkles size={16} /> {editing ? "수정 내용 저장" : "기록 남기기"}
            </Button>
          </div>
        </div>
      )}
